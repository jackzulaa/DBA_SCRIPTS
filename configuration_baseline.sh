...
# THIS SCRIPT WILL WRITE TWO LOG FILES, ONE FOR EACH DATABASE AND ONE FOR OS CONFIGURATIONS.
# Ver [1.5]
#
# FEATURES:
# ^^^^^^^^
...
CLUSTER_CHECK=Y
                 ...
                 y|Y|yes|YES|Yes)
		 echo ""
		 echo -e "\033[33;5mThe Configuration Baseline is now getting collected for DATABASES ONLY...\033[0m"
		 echo "" ;;
		 *)
		 echo ""
		 echo -e "\033[33;5mThe Configuration Baseline is now getting collected...\033[0m"
		 echo "" ;;
                 esac
...
# Check if MAIL_LIST parameter was set:
		...
		"youremail@yourcompany.com")
         	...
         	echo "The Configuration Baseline will be saved in the current directory but will NOT be sent by E-mail."
                echo "In order to receive the report by email, please EDIT line# 130 by replacing youremail@yourcompany.com with your E-mail address."
                echo ". . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . "
		echo ""
        	esac
...
# Setting ORACLE_SID:
# #########################
for ORACLE_SID in $( ps -ef|grep pmon|grep -v grep|grep -Ev ${EXL_DB}|awk '{print $NF}'|sed -e 's/ora_pmon_//g'|grep -v sed|grep -v "s///g" )
   do
    export ORACLE_SID
...
echo "# DOWNLOAD THE LATEST VERSION OF DBA BUNDLE FROM:"                >> ${DB_BASELINE}
...
echo "Configuration Baseline for DATABASE [${DB_NAME}] was saved to: ${DB_BASELINE}"
...
echo "Configuration Baseline for OPERATING SYSTEM was saved to: ${OS_BASELINE}"
echo ""
