...
cat ${ASM_UTL}|grep -Ev ${EXL_DISK_GROUP}|awk '{ print $1" "$NF }'| while read OUTPUT3
   do
...
cat ${TBSLOG}|grep -Ev ${EXL_TBS} |awk '{ print $1" "$NF }'| while read OUTPUT2
   do
...
# Rename the old log generated by the script (if exists):
 if [ -f ${LOG_DIR}/alert_${ORACLE_SID}_new.log ]
  then
   ...
   # Search for errors:
   #ERRORS=`cat ${LOG_DIR}/diff_${ORACLE_SID}.log | grep 'ORA-\|TNS-' |grep -Ev ${EXL_ALERT_ERR}| tail -1`
   ...
   # Search for errors:
   #ERRORS=`cat ${LOG_DIR}/alert_${ORACLE_SID}_new.log | grep 'ORA-\|TNS-' |grep -Ev ${EXL_ALERT_ERR}| tail -1`
...
grep -v '^\#' $ORATAB |grep -Ev "${EXL_DB}"|grep -Ev "${ALL_RUNNING_INSTANCES}"|grep -v "${DB_NAME_LOWER}:"| grep -v "${DB_NAME_UPPER}:"|  grep -v '^$' | grep "^" | cut -f1 -d':' > ${LOG_DIR}/alldb_DBA_BUNDLE.log
	...
	then
	  ...
	  if [ -f ${LOG_DIR}/alert_${LISTENER_NAME}_new.log ]
	   then
	    ...
	    # Search for errors:
	     #ERRORS=`cat ${LOG_DIR}/diff_${LISTENER_NAME}.log|grep "TNS-"|grep -Ev "${EXL_LSNR_ERR}"|tail -1`
	 ...
	 # If no old logs exist:
	    ...
	    # Just create a new log without doing any comparison:
	     ...
	     echo " " 					>> ${LOG_DIR}/alert_${LISTENER_NAME}_new.log
            ...
            # Search for errors:
              #ERRORS=`cat ${LOG_DIR}/alert_${LISTENER_NAME}_new.log|grep "TNS-"|grep -Ev "${EXL_LSNR_ERR}"|tail -1`
...
# Rename the old log generated by the script (if exists):
 if [ -f ${LOG_DIR}/ggserr_new.log ]
  then
   ...
   # Search for errors:
   #ERRORS=`cat ${LOG_DIR}/diff_ggserr.log | grep 'ERROR' |grep -Ev ${EXL_GG_ERR}| tail -1`
   ...
   # Search for errors:
   #ERRORS=`cat ${LOG_DIR}/ggserr_new.log | grep 'ERROR' |grep -Ev ${EXL_GG_ERR}| tail -1`
